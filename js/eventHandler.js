...
function handleEvents(canvas, ctx, layers, image){
    ...
    const shapeStrokeSizePicked = document.getElementById('shapeStrokeSizePicked')
    const shapeStrokeColorPicked = document.getElementById('shapeStrokeColorPicked')
    ...
    newShape.addEventListener('input', (e) =>{
        let selectedObject = layers.selectedObject;
        if(selectedObject.isShape == true){
            if(selectedObject.type == 'rectangle'){
                selectedObject.fill = e.target.value;
                layers.redrawEverything();
            } else if(selectedObject.type == 'circle'){
                let index = layers.objects.indexOf(selectedObject); 
                console.log(selectedObject.x, selectedObject.y, selectedObject.radius, layers, selectedObject.fill)
                let newCircleImage = createCircleImage(selectedObject.x, selectedObject.y, 
                    selectedObject.radius,
                    layers, e.target.value)
                layers.objects[index] = newCircleImage;
            }
        }
    })
